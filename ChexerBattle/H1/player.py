...
class ExamplePlayer:
    ...
    def action(self):
        ...
        # copy state
        red_hexes = self.state.board.players_hexes['red'].copy()
        green_hexes = self.state.board.players_hexes['green'].copy()
        blue_hexes = self.state.board.players_hexes['blue'].copy()
        new_players_hexes = {'red': red_hexes, 'green': green_hexes, 'blue': blue_hexes}
        new_board = Board(self.colour, self.colours)
        new_board.players_hexes = new_players_hexes
        new_board.re_init()
        root_state = State(new_board.piece_hexes, new_board)
        ...
        # prepare additional evaluation cases
        player_index = 0
    ...
    def MAXN(self, node, depth, current_player):
        ...
        for c_node in node.children():
            ...
            if result[current_player - 1] > best[current_player - 1]:
                ...
                # additional evaluation applied if there is a tie in maxn compare(result[p] > best[p])
    ...
    def evaluation(self, node, player):
        ...
        danger_score = -1 * current_board.danger(self.colours[player - 1], node.all_hexes,
                                                 self.state.board.players_hexes) * w_danger
        exit_score = node.score[player - 1] * w_score
        exit_distance = -1 * current_board.dis_exit(self.colours[current_index]) * w_h
        v = num_pieces + exit_score + exit_distance + danger_score
        return v
    ...
    def additional_evaluation(self, node, player, case):
        ...
        if case == 'danger':
            num = -1 * current_board.danger(colour_compare, node.all_hexes, self.state.board.players_hexes)
        elif case == 'safety':
...
# code modified from part A sample solution
class Board:
    """
    ...
    # return the sum of piece to all friend pieces
    ...
    # calculate the destination of a jump over b
    ...
    # return 1 and destination of jump if jump is valid
    ...
    # calculate the number of player's pieces in danger
    def danger(self, colour, all_hexes, players_hexes):
        self_colour = colour
        piece_hexes = self.players_hexes[self_colour].copy()
        red_hexes = players_hexes['red'].copy()
        green_hexes = players_hexes['green'].copy()
        blue_hexes = players_hexes['blue'].copy()
        new_players_hexes = {'red': red_hexes, 'green': green_hexes, 'blue': blue_hexes}
        new_players_hexes[colour] = piece_hexes

        un_safe = []
        un_safe = set(un_safe)
        for piece in piece_hexes:
            for key_enemy in new_players_hexes:
                if key_enemy != self_colour:
                    for enemy in new_players_hexes[key_enemy]:
                        able_to_jump = self.able_to_jump(enemy, piece, new_players_hexes, all_hexes)
                        enemy_move_to = able_to_jump[1]
                        if able_to_jump[0] == 1:
                            have_protection = 0
                            for piece_friend in piece_hexes:
                                if self.able_to_jump(piece_friend, piece, new_players_hexes, all_hexes) == 1:
                                    have_protection = 1
                                    break
                            ...
                            if have_protection == 0:
                                un_safe.add(piece)
                                break
                            # piece is unsafe, but have friend piece protection, check if enemy is threaten by friend
                            else:
                                changed_pieces_pieces = piece_hexes.copy()
                                changed_enemy_pieces = new_players_hexes[key_enemy].copy()
                                changed_players_hexes = new_players_hexes.copy()
                                ...
                                for friend in changed_players_hexes[self_colour]:
                                    able_to_jump = self.able_to_jump(friend, enemy_move_to, changed_players_hexes,
                                                                     all_hexes)
                                    if able_to_jump[0] == 1:
                                        enemy_not_safe = 1
                                        break
    ...
    # change the colour of board and update blocks
...
class State:
    ...
    # return colour updated actions successors
    def colour_actions_successors(self):
        successors = self.actions_successors()
        for i in successors:
            # copy board
            red_hexes = i[1].board.players_hexes['red'].copy()
            green_hexes = i[1].board.players_hexes['green'].copy()
            blue_hexes = i[1].board.players_hexes['blue'].copy()
            new_players_hexes = {'red': red_hexes, 'green': green_hexes, 'blue': blue_hexes}
