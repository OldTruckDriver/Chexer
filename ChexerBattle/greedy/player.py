...
class ExamplePlayer:
    ...
    def action(self):
        ...
        new_board = Board(self.colour)
        new_board.players_hexes = new_players_hexes
        new_board.re_init()
        new_state = State(new_board.piece_hexes, new_board)

        score = {1: 0, 2: 0, 3: 0}
        root = Node(self.colour, ("PASS", None), new_state, root_player, [(4, ("PASS", None))], score)
    ...
    def evaluation(self, node, root_colour):
        v = node.state.board.evaluation(root_colour)
...
class Node:
    def __init__(self, colour, action, state, player, actions, score):
        self.colour = colour
        self.state = state
        self.player = player
        self.next_player = 0
...
class Board:
    ...
    def evaluation(self, root_colour):
        if root_colour == 'red':
...
class State:
    ...
    def colour_actions_successors(self):
        ...
        for i in successors:
            ...
            new_players_hexes = {'red': red_hexes, 'green': green_hexes, 'blue': blue_hexes}
            new_board = Board(i[1].board.colour)
            new_board.players_hexes = new_players_hexes
            new_board.re_init()
