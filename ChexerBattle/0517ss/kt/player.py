...
class ExamplePlayer:
    ...
    def evaluation(self, node, root_colour, current_player):
        v = node.state.board.evaluation(root_colour, node.all_nodes, current_player)

        if len(node.state.board.players_hexes[self.colour]) + node.score[1] > 6 - self.score:
            w_score = 20100
        ...
        v_3 = -100
        if current_player == 1:
            v_1 = v[0] + node.score[1] * w_score + node.killed[1]
        if current_player == 2:
            v_2 = v[1] + node.score[2] * w_score + node.killed[2]
        if current_player == 3:
            v_3 = v[2] + node.score[3] * w_score + node.killed[3]
        v = (v_1, v_2, v_3)
        # print(node.score)
        return v
...
class Board:
    ...
    def evaluation(self, root_colour, all_nodes, current_player):
        colour_1 = 'red'
        colour_2 = 'green'
        colour_3 = 'blue'
        if root_colour == 'red':
        ...
        if current_player == 1:
            ...
            h_1 = self.h(colour_1)
            v_1 = num_1 * 10000 - h_1 * 10 - d_1 * 100 + safe_1
            v_2 = -100
            v_3 = -100
        if current_player == 2:
            ...
            h_2 = self.h(colour_2)
            v_1 = -100
            v_2 = num_2 * 10000 - h_2 * 10 - d_2 * 100 + safe_2
            v_3 = -100
        if current_player == 3:
            ...
            v_1 = -100
            v_2 = -100
            v_3 = num_3 * 10000 - h_3 * 10 - d_3 * 100 + safe_3
        # print(h_1, h_2, h_3)
