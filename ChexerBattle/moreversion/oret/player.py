...
class ExamplePlayer:
    ...
    def MAXN(self, node, depth, current_player):
        ...
        for c_node in node.children():
            # if current_player == 1:
            #     print(c_node.state.board.players_hexes)
            ...
            #     if result[2] > 40000:
            # if current_player == 3:
            #     print(current_player, best, result)
            #     print(best_node.state.board.players_hexes)
            #     print(result_node.state.board.players_hexes)
    ...
    def evaluation(self, node, player):
        ...
        # killed_score = node.killed[player - 1] * w_k
        exit_score = node.score[player - 1] * w_score
        exit_distance = -1 * current_board.dis_exit(self.colours[current_index]) * w_h
        # safety = current_board.safety(self.colours[player - 1])
        v = num_pieces + exit_score + exit_distance + danger_score
        # v = num_pieces + exit_distance + exit_score
        # if player == 1:
        #     print(current_board.players_hexes)
        #     print(len(current_board.players_hexes[self.colours[current_index]]))
        #     print(v)
        # if player == 3:
        #     if num_pieces > 40000:
        #         print(player, num_pieces, self.colours[current_index], current_board.players_hexes[self.colours[current_index]])
            # print('num_', num_pieces)
            # print(current_board.players_hexes)
        ...
        for piece in piece_hexes:
            for key_enemy in new_players_hexes:
                    ...
                    for enemy in new_players_hexes[key_enemy]:
                        ...
                        if able_to_jump[0] == 1:
                            ...
                            # piece is unsafe, but have friend piece, check if enemy is threaten by friend
                                ...
                                if colour == 'green':
                                    print('cgd', changed_players_hexes)
                                enemy_not_safe = 0
                ...
                if piece in un_safe:
                    break
            # if colour == 'green':
            #     print(new_players_hexes)
            #     print(len(un_safe), un_safe)
        return len(un_safe)
