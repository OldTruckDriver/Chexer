    ...
    def MAXN(self, node, depth, current_player, root_colour):
        ...
        for c_node in node.children():
            ...
            if result[current_player - 1] > best[current_player - 1]:
                ...
                case2 = 'safety'
                case3 = 'killed'
                result_ae = self.additional_evaluation(result_node, evaluate_player, case1)
                best_danger_ae = self.additional_evaluation(best_node, evaluate_player, case1)
                if result_ae > best_danger_ae:
                    best_node = result_node
                elif result_ae == best_danger_ae:
                    result_ae = self.additional_evaluation(result_node, evaluate_player, case2)
                    best_ae = self.additional_evaluation(best_node, evaluate_player, case2)
                    if result_ae > best_ae:
                        best_node = result_node
                    elif result_ae == best_ae:
                        result_ae = self.additional_evaluation(result_node, evaluate_player, case3)
                        best_ae = self.additional_evaluation(best_node, evaluate_player, case3)
                        if result_ae > best_ae:
                            best_node = result_node
        if depth == 3:
