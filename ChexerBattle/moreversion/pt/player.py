...
class ExamplePlayer:
    ...
    def MAXN(self, node, depth, current_player, root_colour):
        ...
        for c_node in node.children():
            ...
            if result[current_player - 1] > best[current_player - 1]:
                ...
                ae_result = self.additional_evaluation(result_node, result)
                ae_best = self.additional_evaluation(best_node, best)
                if ae_result > ae_best:
                    best_node = result_node
        if depth == 3:
    ...
    def evaluation(self, node, root_colour, current_player):
        ...
        v_3 = -100
        if current_player == 1:
            v_1 = v[0] + node.score[1] * w_score1
        if current_player == 2:
            v_2 = v[1] + node.score[2] * w_score2
        if current_player == 3:
            v_3 = v[2] + node.score[3] * w_score3
        v = (v_1, v_2, v_3)
        # print(node.score)
        return v

    # more evaluation made if there is a tie
    def additional_evaluation(self, node, value):
        current_player = node.player
        current_board = node.state.board
        colour_compare = current_board.colour
        v = value[current_player - 1]
        w_kill = 100
        w_danger = 10
        w_safety = 10
        num_killed = node.killed[current_player]
        num_danger = current_board.danger(colour_compare, node.all_nodes)
        num_safety = current_board.safety(colour_compare)
        v = v + num_killed * w_kill - num_danger * w_danger + num_safety * w_safety
...
class Board:
    ...
    def evaluation(self, root_colour, all_nodes, current_player, score):
        ...
        w_h2 = 1
        w_h3 = 1
        if score[self.colour_1] + len(self.players_hexes[self.colour_1]) >= 4:
            w_h1 = 1000
        elif score[self.colour_2] + len(self.players_hexes[self.colour_2]) >= 4:
            w_h2 = 1000
        elif score[self.colour_3] + len(self.players_hexes[self.colour_3]) >= 4:
            w_h3 = 1000
        if current_player == 1:
            num_1 = len(self.players_hexes[self.colour_1])
            safe_1 = self.safety(self.colour_1)
            d_1 = self.danger(self.colour_1, all_nodes)
            h_1 = self.dis_exit(self.colour_1)
            if len(self.players_hexes[self.colour_1]) > 4:
            ...
            num_2 = len(self.players_hexes[self.colour_2])
            safe_2 = self.safety(self.colour_2)
            d_2 = self.danger(self.colour_2, all_nodes)
            h_2 = self.dis_exit(self.colour_2)
            v_1 = -100
            if len(self.players_hexes[self.colour_2]) > 4:
                v_2 = num_2 * 10000 - h_2 * w_h2 - d_2 * 100 + safe_2
            else:
                v_2 = num_2 * 10000 - h_2 * w_h2 - d_2 * 100 + safe_2 * 10
            v_3 = -100
        elif current_player == 3:
            num_3 = len(self.players_hexes[self.colour_3])
            safe_3 = self.safety(self.colour_3)
            d_3 = self.danger(self.colour_3, all_nodes)
            h_3 = self.dis_exit(self.colour_3)
            v_1 = -100
            v_2 = -100
            if len(self.players_hexes[self.colour_3]) > 4:
