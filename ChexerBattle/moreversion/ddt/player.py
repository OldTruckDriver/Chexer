...
class ExamplePlayer:
    ...
    def MAXN(self, node, depth, current_player, root_colour):
        ...
        best = (-100, -100, -100)
        best_node = node
        best_case_result = [-1000000, -1000000, -1000000]
        for c_node in node.children():
            ...
            if result[current_player - 1] > best[current_player - 1]:
                ...
                # num_pieces = [0, 0, 0]
                num_pieces = len(self.board.players_hexes[self.colour])
                if num_pieces < 4:
                    cases = ('killed', 'safety', 'danger')
                elif num_pieces == 4:
                    cases = ('safety', 'killed')
                elif num_pieces > 4:
                    cases = ('danger', 'safety', 'killed')
                case_index = 0
                for case in cases:
                    result_ae = self.additional_evaluation(result_node, evaluate_player, case)
                    if best_case_result[case_index] == -1000000:
                        best_ae = self.additional_evaluation(best_node, evaluate_player, case)
                        best_case_result[case_index] = best_ae
                    else:
                        print(best_case_result)
                    if result_ae > best_case_result[case_index]:
                        # print(case)
                        best_node = result_node
                        best_case_result[case_index] = result_ae
                        break
